...
export default function RagChatbot({ context = '', stateName = '', artisanName = '', compact = false }) {
  ...
  return (
    <div className={`rc ${compact ? 'rc--compact' : ''}`}>
      ...
      {/* ── Messages ─────────────────────────────────────────────── */}
      <div ref={messagesRef} className="rc__messages" onScroll={handleMessagesScroll}>
        {messages.map((m, i) => (
      ...
      {!pinnedToBottom && (
        <div className="rc__scroll-note">
          Scroll for earlier answers
        </div>
      )}
